...
def db_first(request):
    ...
    question_types, companies, major_larges, major_smalls, job_larges, job_smalls, recommend_types = dbinit(data_path)
    
    # question_type
    for i in range(len(question_types)):
        instance = QuestionType()
        if i == 0:
            instance.question_type_id = 1000001
    ...
    for i in range(len(major_larges)):
        instance = MajorLarge()
        if i == 0:
            instance.major_large_id = 3000001
    ...
    for i in range(len(major_smalls)):
        instance = MajorSmall()
        if i == 0:
            instance.major_small_id = 4000001
